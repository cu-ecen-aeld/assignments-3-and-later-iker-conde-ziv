...
echo "Testing 10 element buffer"
echo 10 > /sys/module/aesdchar/parameters/circular_buffer_size
comparefile=`mktemp`
...
echo "TEST OK: 10 element buffer"

echo "Testing 20 element buffer"
echo 20 > /sys/module/aesdchar/parameters/circular_buffer_size
comparefile=`mktemp`
test_send_socket_string "swrite1" ${comparefile}
test_send_socket_string "swrite2" ${comparefile}
test_send_socket_string "swrite3" ${comparefile}
...
test_send_socket_string "swrite9" ${comparefile}
test_send_socket_string "swrite10" ${comparefile}
test_send_socket_string "swrite11" ${comparefile}
test_send_socket_string "swrite12" ${comparefile}
test_send_socket_string "swrite13" ${comparefile}
test_send_socket_string "swrite14" ${comparefile}
test_send_socket_string "swrite15" ${comparefile}
test_send_socket_string "swrite16" ${comparefile}
test_send_socket_string "swrite17" ${comparefile}
test_send_socket_string "swrite18" ${comparefile}
test_send_socket_string "swrite19" ${comparefile}
test_send_socket_string "swrite20" ${comparefile}
echo "TEST OK: 20 element buffer"

echo "Testing seekto command"
seek_result=`mktemp`
...
if [ ${rc} -eq 0 ]; then
	echo "TEST OK: ioc seekto command for offset 0,2"
fi
...
check_output ${seek_result} ${comparefile}
if [ ${rc} -eq 0 ]; then
	echo "TEST OK: ioc seekto command for offset 8,6"
fi

echo "Testing buffer size limits (1 <= size <= 32)"
echo "Testing 0 buffer size"
echo 0 > /sys/module/aesdchar/parameters/circular_buffer_size
if [ $? -eq 0 ]; then
  echo "Error in test: 0 buffer size should not be valid"
  rc=-1
fi
echo "TEST OK: 0 buffer size"

echo "Testing 33 buffer size"
echo 33 > /sys/module/aesdchar/parameters/circular_buffer_size
if [ $? -eq 0 ]; then
  echo "Error in test: 33 buffer size should not be valid"
  rc=-1
fi
echo "TEST OK: 33 buffer size"
